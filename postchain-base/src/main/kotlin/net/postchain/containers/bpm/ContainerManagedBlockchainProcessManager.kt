...
class ContainerManagedBlockchainProcessManager(
    ...
    private fun startContainerChain(chainId: Long): BlockchainRid? {
        val process = createProcess(chainId)
    ...
    override fun shutdown() {
        val chainIds = containerProcesses.keys.toSet()
        chainIds.forEach { stopBlockchain(it) }
        super.shutdown()
    }
    ...
    override fun getLaunchedBlockchains(): Set<Long> {
        return super.getLaunchedBlockchains() + containerProcesses.keys
    }
}
