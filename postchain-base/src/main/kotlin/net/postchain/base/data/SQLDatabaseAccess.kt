...
//        return "c${ctx.chainID}.$table" // Will be uncommented later
    ...
    protected abstract fun cmdCreateTableSnapshots(ctx: EContext): String
    ...
    protected abstract fun cmdInsertSnapshots(ctx: EContext): String
    ...
    protected abstract fun cmdCreateDescribeTableFunction(ctx: EContext): String
    ...
    override fun insertSnapshot(ctx: EContext, rootHash: ByteArray, height: Long): Long {
        queryRunner.update(ctx.conn, cmdInsertSnapshots(ctx), rootHash, height, ctx.nodeID)

        val sql = "SELECT snapshot_iid FROM ${tableSnapshots(ctx)} " +
                "WHERE root_hash = ? AND block_height = ? AND node_id = ?"
    ...
    override fun initializeBlockchain(ctx: EContext, blockchainRid: BlockchainRid) {
        ...
        queryRunner.update(ctx.conn, cmdCreateDescribeTableFunction(ctx))
    ...
    override fun getDataInRange(ctx: EContext, tableName: String, limit: Int, offset: Long, original: Long): List<RowData> {
        ...
        var rows = queryRunner.query(ctx.conn, query, mapListHandler, original, offset + 1, offset + limit)
        return rows.map { row ->
            val rowId = row["row_id"] as Long
    ...
    private fun getTableDefinition(ctx: EContext, tableName: String): List<TableDefinition> {
        ...
        """.trimIndent()
        val rows = queryRunner.query(ctx.conn, sql, mapListHandler, ctx.conn.schema, tableName)
    ...
    override fun getPeerInfosInRange(ctx: EContext, limit: Int, offset: Long, original: Long): List<RowData> {
        ...
        val TABLE_PEERINFOS = tablePeerinfos()
