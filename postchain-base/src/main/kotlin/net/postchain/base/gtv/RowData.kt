...
data class RowData(val id: GtvInteger, val tableName: GtvString, val data: GtvArray<Gtv>, val ignoreIndex: GtvArray<GtvInteger> = GtvArray(arrayOf())): Comparable<RowData>, Serializable {
    @Transient
    ...
    private fun toHashGtv(): GtvArray<Gtv> {
    ...
    companion object {
        fun fromGtv(gtv: GtvArray<Gtv>): RowData {
            return RowData(
                    gtv[0] as GtvInteger,
                    gtv[1] as GtvString,
                    gtv[2] as GtvArray<Gtv>,
                    gtv[3] as GtvArray<GtvInteger>
            )
        }
