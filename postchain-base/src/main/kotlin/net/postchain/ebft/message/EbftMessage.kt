    ...
    companion object {
        fun decode(bytes: ByteArray, version: Long): EbftMessage {
            ...
            return when (val topic = data[0].asInteger().toInt()) {
                ...
                MessageTopic.GETPROPOSEDBLOCK.value -> GetProposedBlock(data[1].asByteArray())
                MessageTopic.PROPOSEDBLOCK.value -> ProposedBlock(data[1].asByteArray(), data[2].asArray().map { it.asByteArray() })
