...
class ForceReadOnlyMessageProcessor(
    ...
    fun processMessages() {
        for ((peerId, _, message) in communicationManager.getPackets()) {
            try {
                when (message) {
                    is GetBlockAtHeight -> if (message.height <= blockHeight) sendBlockAtHeight(peerId, message.height)
                    ...
                    is GetBlockSignature -> {
                        ...
                        if (height != -1L && height <= blockHeight) sendBlockSignature(peerId, message.blockRID)
                    }
