...
class BaseBlockchainEngineMetrics(chainIID: Long, blockchainRid: BlockchainRid, transactionQueue: TransactionQueue): Closeable {
    private val transactionQueueSizeGauge: Meter = Gauge.builder("transaction.queue.size", transactionQueue) { transactionQueue.getTransactionQueueSize().toDouble() }
    ...
    override fun close() {
        Metrics.globalRegistry.remove(transactionQueueSizeGauge)
    }
}
