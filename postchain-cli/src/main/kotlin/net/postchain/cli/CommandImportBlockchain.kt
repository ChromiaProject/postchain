...
class CommandImportBlockchain : CliktCommand(name = "import-blockchain", help = "Import a blockchain from file") {
    ...
    override fun run() {
        ...
        StorageBuilder.buildStorage(appConfig).use { storage ->
            val jobId = ImporterExporter.createImportJob(storage, chainId, configurationsFile = configurationsFile, blocksFile = blocksFile, incremental)
            echo("Import job $jobId created")
        }
    }
}
