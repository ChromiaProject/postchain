...
class CommandStart : CliktCommand(name = "start", help = "Starts a node with a given configuration") {

    private val nodeConfigFile by nodeConfigOption()

    private val blockchainConfigFile by blockchainConfigOption()
    private val chainIDs by chainIdOption().default(0)
    ...
    override fun run() {
        if (blockchainConfigFile != null) {
            runStorageCommand(nodeConfigFile, chainIDs) {
        ...
        CliExecution.runNode(nodeConfigFile, listOf(chainIDs), debug)
    }
}
