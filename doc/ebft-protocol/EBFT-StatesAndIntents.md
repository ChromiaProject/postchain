...
| `HaveBlock` | While waiting for consensus to have fetched block from primary we reside in `HaveBlock` state. | If node is primary then enter when block is built, otherwise when received proposed block from primary. |
...
| `FetchProposedBlockIntent`   | `WaitBlock`    | If primary node has built a block, indicated by `Status` message from primary, we enter `FetchProposedBlockIntent` to fetch the proposed block.                         |
