...
| `blockchain_ancestors.<brid_X>` | List of blockchain ancestors [<node_id_Y>:<brid_Z>] | String | | `POSTCHAIN_CONFIG_SEND_INTERVAL_MS` |
...
| `api.enable_tls` | Enable TLS. | Boolean | false | `POSTCHAIN_API_TLS_ENABLE` |
| `api.tls_certificate` | Path to certificate file. | String | "" | `POSTCHAIN_API_TLS_CERTIFICATE` |
| `api.tls_certificate.password` | Certificate file password. | String | "" | `POSTCHAIN_API_TLS_CERTIFICATE_PASSWORD` |
| `api.request-concurrency` | Number of incoming HTTP requests to handle concurrently. | Int | `database.readConcurrency` | `POSTCHAIN_API_REQUEST_CONCURRENCY` |
