 ...
 */
public fun serializeGtvToByteArray(gtv: Gtv): ByteArray {
    return when (gtv) {
...
public abstract class GtvMerkleHashCalculatorBase(digester: Digester?) : MerkleHashCalculator<Gtv, GtvPathSet>(digester) {
 ...
 */
public class GtvMerkleHashCalculatorV1(digester: Digester) : GtvMerkleHashCalculatorBase(digester) {
    ...
    public constructor(cryptoSystem: CryptoSystem) : this(cryptoSystem as Digester)

    override fun getHashSummaryFactory(): GtvMerkleHashSummaryFactory = summaryFactory
}

/**
 * The calculator intended to be used is production for trees that hold [Gtv]
 */
public class GtvMerkleHashCalculatorV2(digester: Digester) : GtvMerkleHashCalculatorBase(digester) {
...
public fun makeMerkleHashCalculator(version: Long): GtvMerkleHashCalculatorBase = when (version) {
